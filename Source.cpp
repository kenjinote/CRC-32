...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_CREATE:
		hEdit = CreateWindowEx(WS_EX_CLIENTEDGE, TEXT("EDIT"), 0, WS_VISIBLE | WS_CHILD | ES_AUTOHSCROLL | ES_READONLY, 0, 0, 0, 0, hWnd, (HMENU)IDOK, ((LPCREATESTRUCT)lParam)->hInstance, 0);
	...
	case WM_DROPFILES:
	{
		...
		if (nFileCount == 1)
		{
			...
			DWORD dwCrc32;
			if (FileCrc32Assembly(szFilePath, dwCrc32) == 0)
			{
				TCHAR szHash[256];
				wsprintf(szHash, TEXT("%d"), dwCrc32);
				SetWindowText(hEdit, szHash);
			}
		}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPreInst, LPSTR pCmdLine, int nCmdShow)
{
	...
	RECT rect = { 0, 0, 320, 280 };
	const DWORD dwStyle = WS_CAPTION | WS_SYSMENU | WS_THICKFRAME | WS_MINIMIZEBOX;
	AdjustWindowRect(&rect, dwStyle, 0);
	HWND hWnd = CreateWindow(
		szClassName,
		TEXT("ドロップされたファイルのCRC32を計算"),
		dwStyle,
		CW_USEDEFAULT,
		0,
